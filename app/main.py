...
st.markdown(
    """
1. **문서 업로드** : Slack / 가이드 / 카톡 / Excel 자료 업로드
2. **문서 색인** : 새 파일만 색인 (옵션: Excel 한국어 요약 생성)
3. **업무 QA** : 자연어로 업무 질문 후 근거 기반 답변
4. **검색 테스트** : LLM 호출 없이 검색 결과만 확인 (비용 절약)
5. **API 상태 확인** : Gemini API 연결 / 모델 목록 확인
6. **Excel 요약 관리** : Excel 한국어 요약 미리보기 / 재생성
"""
)
